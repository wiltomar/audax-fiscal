...
uses
  System.SysUtils, System.Character, Lib.Sistema.Tipos;
...
procedure ShellOpenSite(Site: string);
begin
  {$IFDEF MSWINDOWS}ShellExecute(0, 'open', PChar(Site), nil, nil, SW_SHOWMAXIMIZED);{$ENDIF}
end;

procedure ShellOpenFile(const FileName: string);
var
  Ext: string;
  {$IFDEF MSWINDOWS}SEI: TShellExecuteInfo;{$ENDIF}
begin
...
begin
  ...
  if CreateGuid(ID) <> S_OK then
    Lib.Excecoes.ObjetoNaoIniciado('GUID');
